    ...
    <table>
        ...
        <?php
            while(($registro=$opcao->resultado->FetchNextObject())){
                if($registro->CODIGO==13){
        ?>
                    <td>
                        <a href="#" onclick="<?php echo $registro->LETRA.'()';?>">
                            <fieldset class="classe<?php echo $registro->CODIGO?>">
                                <?php echo ($registro->LETRA); ?><!-- chama a função javascript no arquivo principal.php-->  
                            </fieldset>
                        </a>
                    </td>
                    </tr><tr><!-- NÃO APAGAR... SERVE PARA PULAR A LINHA APÓS A 13º LETRA-->
        <?php
                } else {
        ?>
                    <td>
