...
<div id="pagina"> 
            ...
            <form class="formulario" method="post" action=""> 
                ...
                <input class="btn" type ="submit" name="enviar" value="Pesquisar"/>
                  ...
                  <?php
                                ...
                                $sql_usu=("select usu_nome from usuarios where USU_CODIGO = ".$usuario);
                                $resultado = $con->banco->Execute($sql_usu);
                                if($registro = $resultado->FetchNextObject()){
								    
                                    echo '<table><tr> <td class="valor_a_pagar">'.$registro->USU_NOME.'</td>';
									echo '</tr></table>';
                                }
                    ?> 
               ...
               <form class="formulario" method="post" action="usuadm.php?pa=itens_caixa">
                ...
                <?php
                    ...
                    if($registro = $resultado->FetchNextObject()){
                        ...
                        else if($registro->TP_INSCRICAO=='Inscrição com 1 (um) minicurso do evento e certificado de participação + Refeição(Almoço)'){
                            $taxa = '5.00';
                            echo '<h1 class="valor_a_pagar">'.$registro->TP_INSCRICAO.' R$:  '.number_format($taxa,2,',','.').'</h1><hr />';                                       
                        }
                        else if($registro->TP_INSCRICAO=='Inscrição com 2 (dois) minicursos do evento e certificado de participação + Refeição(Almoço)'){
                            $taxa = '5.00';
                    ...
                    $sql=("SELECT sum( mcso_valor ) AS soma FROM itens_minicursos INNER JOIN usuarios ON IC_USU_CODIGO = USU_CODIGO INNER JOIN minicursos ON IC_MCSO_CODIGO = MCSO_CODIGO WHERE MCSO_STATUS = 'Ativo' AND USU_CODIGO =".$usuario);
                    $resultado = $con->banco->Execute($sql);           
            ...
            <input class="btn" type="submit" name="credenciar" value="Pagamento"/>
            </form>
