...
class usuarios_classe 
  {
	  ...
	  function listar()
	  {    
	        $sql = "select * from usuarios where USU_CODIGO = ".$_SESSION['codigo'];
	  ...
	  {	       
	        $sql = "update usuarios set  USU_EMAIL = '".$_REQUEST['usu_email']."', USU_SEXO ='".$_REQUEST['usu_sexo']."', USU_DATA_NASC ='".$_REQUEST['usu_data_nasc']."', USU_CELULAR ='".$_REQUEST['usu_celular']."', USU_TELEFONE ='".$_REQUEST['usu_telefone']."', USU_ENDERECO = '".$_REQUEST['usu_endereco']."', USU_CIDADE ='".$_REQUEST['usu_cidade']."', USU_UF ='".$_REQUEST['usu_uf']."', USU_CEP ='".$_REQUEST['usu_cep']."',USU_PORTADOR ='".$_REQUEST['usu_portador']."' WHERE USU_CODIGO = ".$_SESSION['codigo'];
			   if ($this->resultado = $this->con->banco->Execute($sql))
			   {
	  ...
	  {	       
          ...
          function listar_modalidade()
	   {
           ...
           function gravar_alterar_senha()
	  {	
               $senha_atual = md5($_REQUEST['senha_atual']);
               $nova_senha = md5($_REQUEST['nova_senha']);
               $repetir_senha = md5($_REQUEST['repetir_senha']);
               
               $sql = "SELECT USU_CODIGO,USU_SENHA FROM usuarios where USU_SENHA = '$senha_atual' and USU_CODIGO = ".$_SESSION['codigo'];
	       $this->resultado = $this->con->banco->Execute($sql);
               if($this->registros = $this->resultado->FetchNextObject()){
                     if(($this->registros->USU_SENHA==$senha_atual)and($nova_senha==$repetir_senha)){
                           $sql = "update usuarios set USU_SENHA = '$nova_senha' WHERE USU_CODIGO = ".$_SESSION['codigo'];
                                if ($this->resultado = $this->con->banco->Execute($sql))
                                {
                                    alerta("Alterado com sucesso");
                                    return true;
                        ...
                        {alerta("As novas senhas informadas não são iguais!");}
 
	        
	  }else {alerta("A senha atual é invalida!");}
