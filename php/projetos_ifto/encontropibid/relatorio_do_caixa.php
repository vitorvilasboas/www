...
<tr><td colspan="3"><label> Relatório de Caixa</label></td></tr>
    ...
    <tr>
        <td class="th100t">Código</td><td class="th600t">Nome</td><td class="th200t">Valor R$</td>
           <?php
              $sql=("select usu_codigo,usu_nome,part_valor_pago FROM caixa INNER JOIN usuarios ON PART_USU_CODIGO = USU_CODIGO order by usu_nome");
            ...
            <tr>
                <td class="td800" colspan="3">
                    ...
                    $sql=("select sum(part_valor_pago) as soma FROM caixa INNER JOIN usuarios ON PART_USU_CODIGO = USU_CODIGO");
                    $resultado = $con->banco->Execute($sql);           
