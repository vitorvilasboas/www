...
<div id="pagina"> 
            ...
            <form class="formulario" method="post" action="index.php?pa=funadm/mcursopart">              
                    <?php
                        ...
                        $sql=("select usu_codigo,usu_nome,mcso_codigo, mcso_horario,mcso_valor, mcso_titulo, mcso_condutor, mcso_resumo FROM itens_minicursos INNER JOIN usuarios ON IC_USU_CODIGO = USU_CODIGO INNER JOIN minicursos ON IC_MCSO_CODIGO = MCSO_CODIGO WHERE MCSO_ANO = '".date('Y')."' and USU_CODIGO = '".$_SESSION['codigo']."'");
               ...
               <table class="tabelas">  
                ...
                <?php    
                    ...
                    $sql=("select sum(mcso_valor) as soma FROM itens_minicursos INNER JOIN usuarios ON IC_USU_CODIGO = USU_CODIGO INNER JOIN minicursos ON IC_MCSO_CODIGO = MCSO_CODIGO WHERE MCSO_ANO = '".date('Y')."' and USU_CODIGO = '".$_SESSION['codigo']."'");
                    $resultado = $con->banco->Execute($sql);           
