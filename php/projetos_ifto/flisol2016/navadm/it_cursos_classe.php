...
class it_cursos_classe{
	  ...
	  function it_cursos_classe(){
          ...
          function listar_minicursos(){
              $retorna = '';   
	      $sql = "select * from minicursos where MCSO_STATUS= 'Ativo' order by MCSO_TITULO";
          ...
          function lista_participantes(){
                ...
                $sql=("select  usu_codigo,usu_nome,ic_situacao from usuarios inner join  itens_minicursos on  IC_USU_CODIGO= USU_CODIGO where IC_MCSO_CODIGO='$minicurso' order by USU_NOME");
                $this->resultado=$this->con->banco->Execute($sql);
          }
}
