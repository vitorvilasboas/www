...
function conteudoGeral($usuario,$evento,$cargaHoraria,$data,$local,$ano){
    ...
    '<div id="conteudo_certificado'.$ano.'"> 
        <p>Certificamos que <b>'.fullUpperc($usuario).'</b> Participou da '.$evento.'
...
function conteudoAvaliador($usuario,$titulo,$apresentador,$participantes,$modalidade,$tipo,$evento,$data,$local,$ano){
    return $conteudo = 
        '<div id="conteudo_certificado'.$ano.'"> 
            <p>Certificamos que <b>'.fullUpperc($usuario).'</b> participou como avaliador(a) do artigo <b>'.fullUpperc($titulo).'</b> com autoria de <b>'.fullUpperc($participantes).'</b> na sessão '.$tipo.', modalidade '.$modalidade.' que foi publicado na '.$evento.', 
            realizado durante o período de  '.$data.' no '.$local.'. </p>  
        </div>';
}
