    ...
    if(@mysql_num_rows(mysql_query("select * from candidatos where can_cpf='{$_POST['cpf_can']}'"))==1){
      ...
      $sql = "insert into candidatos
                (can_nome, can_cpf, can_curso, can_politica, can_rg, can_rg_orgexp, can_rg_uf, can_dtnascimento, can_sexo, 
                  ...
                  can_escrita, can_atespec, can_atespec_descricao, can_insc_processada, can_insc_deferida, can_locprova, can_dtalteracao) values
                ('{$p['nome_can']}', '{$p['cpf_can']}', {$p['opccur_can_1']}, {$p['part_cota']}, '{$p['doc_can']}', '{$p['orgexp_can']}', '{$p['ufdoc_can']}', '$nasc_can', '{$p['sexo_can']}',
                  ...
                  '{$p['hab_can']}', '{$p['vateesp_can']}', '$desc_especial', 1, 0, {$codigo_local_prova},'$datains_can')";
      ...
      if (mysql_query($sql)){
        ...
        $_SESSION['emailcand_session'] = $_POST['mail_can']; 
        ...
        $envio = inscricaoEmail($_POST['mail_can'], $_POST['cpf_can'], $cod_can, $_POST['nome_can'], 'Processo Seletivo 2016/1 do IFTO Campus Araguatins - Cursos Técnicos');

        if( $envio ){
          echo "<script>alert('O numero de inscricao foi encaminhadas ao email cadastrado. Verifique sua caixa de entrada ou caixa de SPAN.');</script>";
        } else {
          echo "<script>alert('Erro ao enviar informacoes de inscricao ao email informado!! Anote o numero de inscricao.');</script>";
        }
